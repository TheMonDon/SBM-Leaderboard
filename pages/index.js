...
export default function Dev() {
  ...
  useEffect(() => {
    fetch("https://api.github.com/repos/TheMonDon/SBM-Leaderboard/stargazers")
      .then((res) => res.json())
      .then((data) => {
    ...
    <div className="bg-background h-screen w-screen flex flex-col overflow-x-hidden">
            ...
            {top.map((user, index) => (
              <div
                className={`flex flex-col items-center m-2 ${
                  top.indexOf(user) === 1 ? "w-1/2" : "w-1/4"
                }`}
                key={index}
              >
                <h1>
                ...
                />
                <p className="italic text-center text-primary-3">
                  {user?.quote}
                </p>
              </div>
            ))}
          ...
          <div className="flex flex-col w-full bg-primary-1 rounded-md">
            {!leaderboardLoading ? (
              leaderboard.map((user, index) => (
                <div
                  className="text-primary-3 w-full p-2 flex flex-row"
                  key={index}
                >
                  <p
                    className={`px-2 ${
                      leaderboard.indexOf(user) <= 2 && "border-[1px]"
                    } ${
                      [
                        "border-yellow-500",
                        "border-silver-500",
            ...
            {loggedIn ? (
              steamLinked ? (
                <div className="flex flex-col">
                  <h1 className="self-center pt-2 text-3xl text-primary-3">
                    {session.user.name}#{session.user.tag}
                  </h1>
                  <p className="self-center text-primary-3 mt-1">
                    Playtime: {Math.floor(playtime / 60)} hours
                  </p>
                  <textarea
                    className="outline-0 rounded-md p-2 m-2 bg-gray-300"
                    ref={quoteRef}
                    maxLength="32"
                  ></textarea>
                  <button className="bg-primary-1 text-primary-3 rounded-md p-2 flex flex-row space-x-2 justify-center mt-2">
                    <IoPushOutline className="h-full" />
                    <p className="h-full" onClick={setQuote}>
                      Update Quote
                    </p>
                  </button>
                  <button
                    ...
                    onClick={() => signOut()}
                  >
                    <ImExit className="h-full" />
                    <p className="h-full">Sign Out</p>
                  </button>
                  {playtime === 0 && (
                    <div className="flex flex-col">
                      <p className="self-center pt-2 text-xl text-primary-3 text-center">
                        No hours? Make sure you have your &quot;Game
                        details&quot; set to &quot;Public&quot; and check the
                        box under the setting in your
                        <a
                          target="_blank"
                  ...
                  <div className="flex flex-col items-center text-primary-3">
                    <h1 title="Star this project on github to get your name on this list!">
                      ⭐
                    </h1>
                ...
                <div>
                  {steamLoading || true ? (
                    ...
                    <div>
                      <h1 className="self-center pt-2 text-3xl text-primary-3 text-center">
                        You must link your steam account to your discord account
                        to continue!
                      </h1>
                    </div>
                  )}
              ...
              <div className="flex flex-col">
                <button
                  className="bg-gray-700 text-primary-3 rounded-md p-2 flex flex-row space-x-2 justify-center"
                  onClick={() => signIn("discord")}
                >
                  <FaDiscord className="h-full" />
      ...
      <div className="flex flex-row justify-center mt-auto">
        <div className="flex flex-col text-primary-3 text-center m-4">
          ...
          <a
            href="https://github.com/TheMonDon/SBM-Leaderboard"
